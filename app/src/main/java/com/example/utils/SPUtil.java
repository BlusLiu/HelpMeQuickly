...
public class SPUtil {
    ...
    public SPUtil(String configName) {
        ...
        try {
            instance = MyApplication.getContext()
                    .getSharedPreferences(configName, Context.MODE_PRIVATE);
        } catch (Exception e) {
            e.printStackTrace();
            SharedPreferences.Editor editor = MyApplication.getContext().getSharedPreferences(configName, 0).edit();
            editor.putString("test_name","liu");
            editor.apply();
            instance = MyApplication.getContext()
                    .getSharedPreferences(configName, Context.MODE_PRIVATE);
        }finally {
